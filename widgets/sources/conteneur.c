...
static GtkAlign _convertir_align(ConteneurAlignement a) {
    switch(a) {
        case ALIGNEMENT_DEBUT: return GTK_ALIGN_START;
        case ALIGNEMENT_FIN:   return GTK_ALIGN_END;
        case ALIGNEMENT_CENTRE:return GTK_ALIGN_CENTER;
        case ALIGNEMENT_REMPLIR: default: return GTK_ALIGN_FILL;
    }
}
...
void conteneur_initialiser(Conteneur *config) {
    ...
    config->marges.haut = 0; config->marges.bas = 0;
    config->marges.gauche = 0; config->marges.droite = 0;

    // Padding (Intérieur) à 0
    config->padding.haut = 0; config->padding.bas = 0;
    config->padding.gauche = 0; config->padding.droite = 0;
