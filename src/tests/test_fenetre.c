...
static void init_chemins(void) {
    ...
    for (char *p = ICON_PATH;  *p; p++) if (*p == '\\') *p = '/';
    for (char *p = IMAGE_PATH; *p; p++) if (*p == '\\') *p = '/';
...
typedef struct { Fenetre *f; } PosData;
...
static void on_fermer_popup(GtkWidget *w, gpointer data) {
    if (GTK_IS_WINDOW(data)) gtk_window_destroy(GTK_WINDOW(data));
}
...
// ════════════════════════════════════════════════════════════
    ...
    struct { const char *id,*txt,*bg,*hov; GCallback cb; } tests[] = {
